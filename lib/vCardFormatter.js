...
(function vCardFormatter() {
    ...
    module.exports = {
        ...
        getFormattedString: function(vCard) {
            ...
            var encodingPrefix = majorVersion >= 3 ? ';CHARSET=ISO-8859-1' : ';CHARSET=ISO-8859-1';
            ...
            if (vCard.iosURL) {
                if (!Array.isArray(vCard.iosURL)) {
                    formattedVCardString += 'item1.URL' + encodingPrefix + ':' + e(vCard.iosURL) + nl();
                    formattedVCardString += 'item1.X-ABLABEL' + encodingPrefix + ':' + e(vCard.label) + nl();
                }
                else {
                    vCard.iosURL.forEach((url, index) => {
                        formattedVCardString += 'item' + (index + 1) + '.URL' + encodingPrefix + ':' + e(url) + nl();
                        formattedVCardString += 'item' + (index + 1) + '.X-ABLABEL' + encodingPrefix + ':' + e(vCard.label[index]) + nl();
                    });
                }
            }
            ...
            if (vCard.customSocialUrls) {
                vCard.customSocialUrls.forEach((socialProfile, index) => {
                    formattedVCardString += 'item' + (index + vCard.label.length) + '.X-SOCIALPROFILE' + encodingPrefix + ':' + e(socialProfile) + nl();
                    formattedVCardString += 'item' + (index + vCard.label.length) + '.X-ABLABEL' + encodingPrefix + ':' + e(vCard.label[index + vCard.label.length - 1]) +nl();
                })
            }
